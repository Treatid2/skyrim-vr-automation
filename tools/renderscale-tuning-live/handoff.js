...
async function startRenderScaleTuningWorker({ tools, runId, buildId, positioningRoot,
    startupReceipts, pluginRoot }) {
    if (!/^[A-Za-z0-9_-]+$/.test(runId) || typeof pluginRoot !== "string") {
    ...
    const request = { variant: "nvidia", runId, buildId, workspace: root,
    ...
    const launched = await tools.exec_command({
        ...
        justification: "Start the authorized persistent NVIDIA assay worker with its local endpoint ownership lock and selected DevBench MCP connection.",
