...
Assert-Test ($entryPointText -match '\$persistentSessionInvalidation\) \{ ''persistent-session-invalidated''' -and $entryPointText -match 'lastSuccessfulObservation = \$lastSuccessfulWaitObservation' -and $entryPointText -match 'Update-InvocationEvidence -State \$failureState[\s\S]{0,100}-Data \$failureData') 'persistent session invalidation returns and journals a distinct state with the last successfully decoded observation'
