...
const liveResult = await runLive({tools,store,notify,variant:"amd",runId,buildId,positioningRoot,startupReceipts:{prepare:prepareEnvelope,positioning:positioningEnvelope},matrix:JSON.parse(support[1].output)});
